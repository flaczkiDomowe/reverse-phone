...
class StandardKeyboardTest extends TestCase
{
    ...
    public function testMakeSentenceFromCode()
    {   $phone=new Phone();
        $keyboard=new StandardKeyboard($phone);
        foreach($this->expectedResults as $key=>$val){
            self::assertEquals($keyboard->makeSentenceFromCode($val),$key);
        }
    }
}
